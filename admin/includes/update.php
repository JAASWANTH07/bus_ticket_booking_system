...
if (isset($_POST['update-bus'])) {
	...
	$query = "UPDATE posts SET post_title='{$title}', post_date='{$date}', post_source='{$source}', post_destination='{$destination}', post_author='{$admin}', post_category_id={$category}, post_via='{$intermediate}', post_via_time='{$via_time}', seat_cost='{$seat_cost}', post_content='{$bus_detail}'
	WHERE post_id=$edit_bus_id ";
...
<form action="" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<input value="<?php echo $seat_cost; ?>" type="text" class="form-control" name="seat_cost" placeholder="Cost per 1 Ticket">
	</div>
	...
	<div class="form-group">
		<input type="submit" class="btn btn-primary" onclick="busUpdate()" name="update-bus" value="Update">
	</div>
</form>
